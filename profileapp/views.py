...
def searchbar(request):
    ...
    if request.method == 'GET':
        ...
        if query:
            ...
            return render(request, 'profileapp/searchbar.html', {'key1':J, "posts": posts, "page": "Search", "suggestions":suggestions})
        else:
            messages.info(request, 'No search result!!!')
            print("No information to show")
    return render(request, 'profileapp/searchbar.html',{"page": "Search", "posts": posts, "suggestions":suggestions})
...
def register(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(request, "profileapp/register.html", {
        ...
        # Attempt to create new user
            ...
            message = render_to_string('profileapp/Account_verification.html', {
            'user': user,
            'domain': current_site,
        ...
        send_mail(
                ...
                message,
                'justin150598photos@gmail.com',
                [email],
                fail_silently=False,
            )
        return redirect('http://127.0.0.1:8000/n/login?command=verification&email='+email)
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(email=email).exists():
            ...
            current_site = get_current_site(request)
            message = render_to_string('profileapp/ResetPassword_email.html', {
                'user': user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),
                'token': default_token_generator.make_token(user),
            ...
            return redirect('forgotPassword')
    return render(request, 'profileapp/Forgot_Password.html')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        return render(request, 'profileapp/ResetPassword.html')
...
def change_password(request):
    ...
    return render(request, "profileapp/Change_Password.html", {
                "suggestions": suggestions,
...
def comp_change_password(request):
    if request.method == 'POST':
        ...
        if success:
            ...
            messages.error(request, 'Password does not match!')
            return redirect('change_password')
    return render(request, 'profileapp/Company_changepass.html')